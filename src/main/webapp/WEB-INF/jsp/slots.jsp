			...
			<div class="col-sm-12 card col-lg-8 pt-2">
				...
				<table class="table table-bordered">
					...
					<tbody>
						...
						<tr>
							<td class="alert-primary">Center Name :</td>
							<td>${center.name }</td>

						</tr>
						<tr>
							<td class="alert-primary">Center Address :</td>
							<td>${center.address }</td>

						</tr>
						<tr>
							<td class="alert-primary">Pincode</td>
							<td>${center.pincode }</td>
						</tr>
						<tr>
							<td class="alert-primary">Age Group</td>
							<td>${age }+</td>
						</tr>
					...
					<tbody>
						<c:choose>
							<c:when test="${sessionList.size() == 0 }">
								<tr>
									<td class="alert-secondary">No Slots Available</td>
								</tr>
							</c:when>
							<c:otherwise>
								<c:forEach items="${sessionList}" var="sessionList">
									<tr>
										<td class="alert-secondary" colspan="4">${sessionList.date}</td>
										...
										<td><c:choose>
												<c:when test="${ sessionList.available_capacity==0 }">
													<div class="alert-danger"
														style="float: left; padding: 5px; border: 1px solid red; border-radius: 5%;">Booked</div>
												</c:when>
												<c:otherwise>
													<div class=" alert-success"
														style="float: left; padding: 5px; border: 1px solid green; border-radius: 5%;">${ sessionList.available_capacity }
														Vaccine Available</div>

												</c:otherwise>
											</c:choose></td>
										<td>${sessionList.vaccine }</td>
										<td>Dose1 : ${sessionList.available_capacity_dose1 }</td>
										<td>Dose2 : ${sessionList.available_capacity_dose2 }</td>
									</tr>
								</c:forEach>
